...
// NewAccountKeeper returns a new AccountKeeperI that uses go-amino to (binary) encode and decode
// concrete sdk.Accounts.  `maccPerms` is a map that takes accounts' addresses as keys, and their
// respective permissions as values. This map is used to construct types.PermissionsForAddress and
// is used in keeper.ValidatePermissions. Permissions are plain strings, and don't have to fit into
// any predefined structure. This auth module does not use account permissions internally, though
// other modules may use auth.Keeper to access the accounts permissions map.
